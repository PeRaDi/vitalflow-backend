...
export class TenantsService {
    ...
    async addContacts(tenantId: number, contactNumbers: number[], contactInfos: string[]): Promise<boolean> {
        const query = 'INSERT INTO tenant_contacts (tenant_id, contact, info) VALUES ($1, $2, $3) RETURNING *;';
    ...
    async deleteContacts(tenantId: number, contactsIds: number[]): Promise<boolean> {
        const query = "DELETE FROM tenant_contacts WHERE tenant_id = $1 AND id = $2 RETURNING *;";
    ...
    async updateContact(tenantId: number, contactId: number, contactNumber: number, contactInfo: string): Promise<boolean> {
        const query = "UPDATE tenant_contacts SET contact = $1, info = $2, updated_at = $3 WHERE tenant_id = $4 AND id = $5 RETURNING *;";
        const params = [contactNumber, contactInfo, new Date(), tenantId, contactId];
